<!-- ========================================== -->
<!-- WIDOK: ODRZUCONE POŁĄCZENIA Z HOSTÓW WEWNĘTRZNYCH -->
<!-- ========================================== -->

<?php
if (!function_exists('ohw_normalize_label')) {
    function ohw_normalize_label($value, $fallback = 'Nieznany') {
...
if (!function_exists('ohw_parse_terms_counts')) {
    function ohw_parse_terms_counts($rawText, $fallbackCount = 0) {
        $items = [];
        ...
        if (empty($items) && trim($rawText) !== '') {
            $items[ohw_normalize_label($rawText)] = max(1, (int)$fallbackCount);
        }
        return $items;
    }
}

if (!function_exists('ohw_add_stat')) {
    function ohw_add_stat(&$bucket, $label, $count) {
        $label = ohw_normalize_label($label);
        $count = max(1, (int)$count);
        $bucket[$label] = ($bucket[$label] ?? 0) + $count;
    }
}

if (!function_exists('ohw_build_hourly_events')) {
    function ohw_build_hourly_events($scan) {
...
if (!function_exists('ohw_render_top_card')) {
    function ohw_render_top_card($title, $items, $icon = 'bar-chart-3', $accent = 'red') {
        arsort($items);
...
foreach ($scans as $scan) {
    ...
    foreach (ohw_parse_terms_counts($scan['dest_country_raw'] ?? ($scan['dest_country'] ?? ''), $events) as $label => $count) {
        ohw_add_stat($destCountryCounts, $label, $count);
    }
    foreach (ohw_parse_terms_counts($scan['source_country_raw'] ?? ($scan['source_country'] ?? ''), $events) as $label => $count) {
        ohw_add_stat($sourceCountryCounts, $label, $count);
    }
    ohw_add_stat($portCounts, $scan['dest_port'] ?? 'Dowolny', $events);
    ohw_add_stat($protocolCounts, $scan['protocol'] ?? 'Nieznany', $events);
    ohw_add_stat($serviceCounts, $scan['service'] ?? 'Nieznana', $events);
}
?>
...
<div class="mb-8 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
    ...
    <div class="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
        <div class="flex items-center justify-between">
            ...
            <div class="rounded-xl bg-blue-50 p-3 text-blue-600"><i data-lucide="shield-check" class="h-6 w-6"></i></div>
        </div>
    </div>
</div>
...
<div class="mb-8 rounded-2xl border border-slate-150 bg-white p-6 shadow-sm">
    <div class="mb-6 flex flex-col justify-between gap-4 border-b border-slate-100 pb-4 sm:flex-row sm:items-center">
        <div>
            ...
            <p class="mt-1 text-xs text-slate-400">Analiza zablokowanych połączeń, portów docelowych, usług, protokołów, krajów i godzin występowania.</p>
        </div>
            ...
            <tbody class="divide-y divide-slate-50 text-xs font-medium" id="scans-table-body">
                ...
                <?php else: ?>
                    <?php foreach ($scans as $index => $scan):
                        ...
                        $hourlyData = ohw_build_hourly_events($scan);
                    ...
                    ?>
                        <tr class="hover:bg-slate-50/50 <?php echo $rowBorder; ?>" id="<?php echo $rowId; ?>">
                            ...
                            <td class="px-4 py-3.5 text-slate-600">
                                <div class="font-semibold text-slate-800"><?php echo htmlspecialchars($scan['destination_position'] ?? 'out'); ?></div>
                        ...
                        <tr id="<?php echo $detailId; ?>" class="hidden bg-slate-50/50" style="display:none;">
                            <td colspan="8" class="p-6">
                                <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
                                    ...
                                    <div class="rounded-xl border border-slate-100 bg-white p-4">
                                        <h4 class="mb-3 flex items-center gap-1 text-xs font-bold uppercase tracking-wider text-slate-900"><i data-lucide="search" class="h-4 w-4 text-blue-500"></i> Analiza celu</h4>
                                        <div class="mb-3 text-[11px] text-slate-500">Host <b><?php echo htmlspecialchars($scan['source_ip'] ?? ''); ?></b> → <b><?php echo htmlspecialchars($scan['dest_ip'] ?? ''); ?>:<?php echo htmlspecialchars($scan['dest_port'] ?? ''); ?></b>, zdarzeń <b><?php echo number_format((int)($scan['events_count'] ?? 0), 0, ',', ' '); ?></b>.</div>
