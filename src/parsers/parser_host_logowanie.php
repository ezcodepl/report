<?php
/**
 * Parser raportu Logsign: próby logowania według hostów.
 * Obsługiwane kolumny:
 ...
 */
class RaportHostLogowanieParser {
    private $filePath;
    private $fileName;
    ...
    private function extractTimeGeneratedList(DOMXPath $xpath, ?DOMNode $cell) {
        ...
        foreach ($this->cellLines($xpath, $cell) as $line) {
            if (preg_match('/(\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2})\s*\(([\d\s,]+)\)/u', $line, $m)) {
                ...
                $list[] = ['datetime' => $m[1], 'count' => max(1, $count), 'hour' => substr($m[1], 11, 2)];
            } elseif (preg_match('/(\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2})/u', $line, $m)) {
                $list[] = ['datetime' => $m[1], 'count' => 1, 'hour' => substr($m[1], 11, 2)];
            }
        }
        return $list;
    }
    ...
    private function isLoginTable(array $headerMap) {
        return $this->idx($headerMap, ['Source.UserName']) !== null
            && $this->idx($headerMap, ['Source.HostName']) !== null
    ...
    public function parse() {
        ...
        foreach ($tables as $table) {
            ...
            if ($bodyRows === false || $bodyRows->length === 0) $bodyRows = $xpath->query('.//tbody/tr', $table);
            ...
            foreach ($bodyRows as $row) {
                ...
                if ($user !== '-') {
                    ...
                    $userCounts[$user] = ($userCounts[$user] ?? 0) + $events;
                }
                if ($sourceIp !== '-') $uniqueIps[$sourceIp] = true;
                if ($sourceHost !== '-') {
                    $uniqueHosts[$sourceHost] = true;
                    $hostCounts[$sourceHost] = ($hostCounts[$sourceHost] ?? 0) + $events;
                }
            }
        }
        ...
        if (!empty($hostCounts)) { arsort($hostCounts); $data['meta']['najaktywniejszy_host'] = array_key_first($hostCounts); }
